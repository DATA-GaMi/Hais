...
replace()
{
        grep -n 'ExecStart=/usr/bin/dockerd -H fd:// --containerd=/run/containerd/containerd.sock' /lib/systemd/system/docker.service
        if [ $? -eq 1 ]
        then
...
grep -n "tcp://0.0.0.0:2376" /lib/systemd/system/docker.service
if [ $? -eq 0 ]
then
	echo "设置已替换完成，不用再次替换."
else
